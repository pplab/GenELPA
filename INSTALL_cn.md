...
示例Makefile是在翰海20系统上编译静态库和测试程序的例子。Makefile的前11行设置了编译参数，旧API和新API的ELPA的链接库，MKL库，旧API和新API的头文件，以及程序本身和MKL的头文件的参数。第13行到第60行是各种测试程序的编译语句。第61行和第73行分别是旧API和新API的库文件编译语句。
