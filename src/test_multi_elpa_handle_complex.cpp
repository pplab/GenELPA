...
void diag(bool isNewRound, MPI_Comm comm, int nev, int narows, int nacols,		  
		  complex<double> *a, complex<double> *b, complex<double> *atmp, complex<double> *btmp, 
		  double* ev, complex<double>* q, int* desc)
{
	...
	Czcopy(subMatrixSize, a, atmp);
    static int DecomposedState;
    if(isNewRound)
    {
		DecomposedState=0;
		Czcopy(subMatrixSize, b, btmp);
    }
...
int main(int argc, char** argv)
{
    ...
    {
        outlog.str("");
        outlog<<"myid "<<myid<<": ELPA_Solver is inited"<<endl;
        cout<<outlog.str();
    }
    ...
    {
        outlog.str("");
        outlog<<"myid "<<myid<<": log level is set as "<<loglevel<<endl;
        cout<<outlog.str();
    }
